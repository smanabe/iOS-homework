...
- (void)viewDidLoad
{
    ...
    [_tableView registerNib:[UINib nibWithNibName:@"TVSCustomCell" bundle:nil] forCellReuseIdentifier:@"nogizaka"];

    _cellForCalcHeight = [_tableView dequeueReusableCellWithIdentifier:@"nogizaka"];

    //TODO : samplData.plist から NSArray を作成しましょう [1]
    NSString *p = [[NSBundle mainBundle] pathForResource:@"sampleData2" ofType:@"plist"];
    //NSLog(@"%@", p);
    self.texts = [NSArray arrayWithContentsOfFile:p];
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGFloat size = [_cellForCalcHeight calculateCellHeightWithText:text];
    return size;
    //return 10;
}
